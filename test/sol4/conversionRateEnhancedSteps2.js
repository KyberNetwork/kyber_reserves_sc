...
contract('ConversionRateEnhancedSteps', function (accounts) {
  before('inits global variables', async function () {
    admin = accounts[0];
    alerter = accounts[1];
    operator = accounts[2];
  ...
  it('should verify get rate returns 0 when qty + total imbalance are above maxTotalImbalance.', async function () {
    ...
    while (totalImbalance + imbalance > -maxTotalImbalance) {
      await convRatesInst.recordImbalance(token, imbalance, lastSetCompactBlock, currentBlock++, {
        from: reserveAddress
      });
  ...
  it('should verify get rate returns 0 when qty + total imbalance are above step with y = -10000', async function () {
    ...
    await convRatesInst.recordImbalance(tokens[tokenInd], new BN(curImbalance[0]).mul(new BN(-1)), 0, currentBlock, {
      from: reserveAddress
    });
  ...
  it('should verify getImbalancePerToken returns correct data as recorded or when overflow.', async function () {
    ...
    Helper.assertEqual(
      newImbalance[0],
      imbalanceBlock0[0],
      'imbalance must be the same when sending 0 as current block'
    );
    Helper.assertEqual(
      newImbalance[1],
      imbalanceBlock0[1],
    ...
    try {
      ...
      await convRatesInst.recordImbalance(tokens[tokenInd], newAddImbalAmount, 0, currentBlock, {
        from: reserveAddress
      });
  ...
  describe('delist token', async () => {
    ...
    beforeEach('add tokens', async () => {
      ...
      for (let i = 0; i < numTokens; ++i) {
        ...
        await convRatesInst.addToken(tokenAddr);
        await convRatesInst.setTokenControlInfo(
          tokenAddr,
          minimalRecordResolution,
          maxPerBlockImbalance,
    ...
    it('delists token - check emptySlotIndex', async () => {
      const REMOVING_INDEX = 15;
      const REMOVING_INDEX2 = 7;
