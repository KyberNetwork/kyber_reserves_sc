...
contract ConversionRatesBiDirection is ICRBiDirection, VolumeImbalanceRecorder, Utils {
    ...
    function fetchRate(
        ...
        if (buy) {
            ...
            // add rate update
            extraBps = int(getRateByteFromCompactData(compactData, token, true)) * 10;
            ...
            // add qty overhead
            extraBps = executeStepFunction(tokenData[token].buyRateQtyStepFunction, imbalanceQty);
            rate = addBps(rate, extraBps);
            ...
            // add rate update
            extraBps = int(getRateByteFromCompactData(compactData, token, false)) * 10;
            rate = addBps(rate, extraBps);
            ...
            // add qty overhead
            extraBps = executeStepFunction(tokenData[token].sellRateQtyStepFunction, imbalanceQty);
            rate = addBps(rate, extraBps);
