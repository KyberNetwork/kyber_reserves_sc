...
contract KyberReserveBiDirection is KyberReserveInterface, Withdrawable, Utils {

    address public kyberNetwork;
    bool public tradeEnabled;
    ICRBiDirection public conversionRatesContract;
    ...
    function KyberReserveBiDirection(
        address _kyberNetwork,
        ICRBiDirection _ratesContract,
        address _admin
    ) public {
        ...
        payable
    {
        ...
        doTrade(srcToken, qty, isSrcQty, destToken, destAddress, conversionRate, validate);
    }
    ...
    function setContracts(
        address _kyberNetwork,
        ICRBiDirection _conversionRates,
    ...
    function getConversionRate(
        ...
        uint rate = conversionRatesContract.fetchRate(token, blockNumber, isBuy, qty, isSrcQty);
        uint destQty = isSrcQty ? calcDstQty(
            getDecimals(src),
            getDecimals(dest),
            qty,
            rate
    ...
    /// @param qty Amount of src or dest token
    /// @param isSrcQty Whether qty specified is in src or dest token
        ...
        returns(bool)
    {
        ...
        if (validate) {
            ...
            if (srcToken == ETH_TOKEN_ADDRESS && isSrcQty)
                require(msg.value == qty);
            else
                require(msg.value == 0);
        }
    ...
    // overriding function here to utilise MAX_RATE change
