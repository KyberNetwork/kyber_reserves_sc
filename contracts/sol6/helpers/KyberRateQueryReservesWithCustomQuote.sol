...
contract KyberRateQueryReservesWithCustomQuote is Utils {

    IERC20Ext public immutable quoteToken;
    uint public immutable quoteTokenDecimals;

    constructor (IERC20Ext _quoteToken) public {
        quoteToken = _quoteToken;
        quoteTokenDecimals = _quoteToken.decimals();
    }
        ...
        external view returns(uint256[] memory pricingRates, uint256[] memory sanityRates)
    {
        ...
        for (uint256 i = 0 ; i < srcs.length ; i++) {
            ...
            pricingRates[i] = conversionRateContract.getRate(
                srcs[i] == quoteToken ? dests[i] : srcs[i],
                block.number,
                srcs[i] == quoteToken ? true : false,
                0);
        }
    }
