...
/// @title KyberReserveMulQuote
/// Allow reserve to work either weth or eth.
/// for working with weth should specify external address to hold weth
/// Allow reserve to work with a primary quote token, and specify secondary quote tokens
...
contract KyberFprReserveV2 is IKyberReserve, Utils, Withdrawable {
    ...
    ICRMulQuotes public conversionRatesContract;
    ...
    constructor(
        ...
        require(_ratesContract != ICRMulQuotes(0), "ratesContract 0");
    ...
    function setConversionRate(ICRMulQuotes _newConversionRate) external onlyAdmin {
        require(_newConversionRate != ICRMulQuotes(0), "conversionRates 0");
    ...
    function getConversionRate(
        ...
        uint256 rate;
        try conversionRatesContract.getRate(src, dest, blockNumber, srcQty) returns(uint256 r) {
            rate = r;
        } catch {
            return 0;
        }
